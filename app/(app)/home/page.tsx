...
import { ThoughtEngine } from '@/components/ThoughtEngine'
...
export default function Home() {
  ...
  // Tag changes trigger fetchNotes directly via changeTag() below — no useEffect needed
  ...
  function mergeNote(note: Note & { _tempId?: string }) {
    ...
    setNotes(curr => {
      ...
      return exists ? withoutTemp.map(n => n.id === clean.id ? { ...n, ...clean } : n) : [clean, ...withoutTemp]
    })
    setAllNotes(curr => {
      const withoutTemp = tempId
        ? curr.filter(n => n.id !== tempId)
